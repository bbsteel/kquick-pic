...
**架构:** 新建 `quick_pic/annotations.py` 作为唯一的数据模型和 Cairo 渲染源。`area_selector.py`（屏幕标注）和 `screenshot.py`（最终图片）都调用 `render_annotations()` 统一绘制。直线/箭头的拖拽预览使用独立函数 `draw_line_preview` / `draw_arrow_preview`。
...
### Task 1: 创建 `quick_pic/annotations.py` — 标注 dataclass

**文件:** 创建 `quick_pic/annotations.py`
...
```bash
uv run python -c "from quick_pic.annotations import LineAnnotation, ArrowAnnotation, SelectionResult; print('OK')"
```
...
```bash
git add quick_pic/annotations.py
...
**文件:** 修改 `quick_pic/annotations.py`
...
```bash
uv run python -c "from quick_pic.annotations import render_annotations, draw_line_preview, draw_arrow_preview; print('OK')"
```
...
**文件:** 修改 `quick_pic/area_selector.py`
...
```bash
uv run python -c "from quick_pic.area_selector import AreaSelector; print('OK')"
```

- [ ] **Step 4: 提交**

```bash
git add quick_pic/area_selector.py
...
```bash
uv run python -c "from quick_pic.screenshot import ScreenshotCapture; print('OK')"
```

- [ ] **Step 5: 提交**

```bash
git add quick_pic/screenshot.py
...
**文件:** 修改 `quick_pic/locales/en.json`、`quick_pic/locales/zh-CN.json`
...
uv run python -c "
import json
json.load(open('quick_pic/locales/en.json'))
json.load(open('quick_pic/locales/zh-CN.json'))
print('OK')
"
```
...
```bash
git add quick_pic/locales/en.json quick_pic/locales/zh-CN.json
...
```bash
uv run python -c "from quick_pic.area_selector import AreaSelector; a = AreaSelector(); print(hasattr(a, '_line_button'))"
```
...
**文件:** 修改 `quick_pic/area_selector.py` — `_on_tool_toggled` 方法（第 557-566 行）
...
```bash
uv run python -c "from quick_pic.area_selector import AreaSelector; a = AreaSelector(); print(hasattr(a, '_relative_line_within_selection'))"
```
...
**文件:** 修改 `quick_pic/area_selector.py` — `_on_draw_overlay` 方法
...
```bash
uv run python -c "from quick_pic.app import QuickPicApp; print('OK')"
```

- [ ] **Step 3: 验证 i18n**

```bash
uv run python -c "from quick_pic.i18n import t; print(t('selector.draw_line')); print(t('selector.draw_arrow'))"
```
...
uv run pytest tests/ -v
uv run python -c "from quick_pic.app import QuickPicApp; print('OK')"
uv run python -c "from quick_pic.i18n import t; print(t('selector.draw_line'), t('selector.draw_arrow'))"
```
