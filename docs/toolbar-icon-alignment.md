...
kuick-pic 截图工具的区域选择器（`area_selector.py`）在用户选区完成后，会在画面底部显示一排工具栏按钮（画框、添加文字、画线、画箭头、数字印章、选择颜色、撤销、确认、取消）。
...
相关代码位置：`kuick_pic/area_selector.py`，函数 `_make_tool_button()`（在 `_setup_overlay()` 内部）。
...
使用 `Gtk.Image.new_from_pixbuf()` 加载固定尺寸（如 20×20）的图标图片，图标文件统一放在 `kuick_pic/icons/` 目录。
