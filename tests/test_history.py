...
def test_list_recent_screenshots_orders_newest_first(tmp_path: Path):
    older = tmp_path / "kuick-pic-2020-01-01-00-00-00.png"
    newer = tmp_path / "kuick-pic-2026-01-01-00-00-00.png"
...
def test_list_recent_screenshots_respects_limit(tmp_path: Path):
    paths = []
    for i in range(7):
        p = tmp_path / f"kuick-pic-2026-01-01-00-00-0{i}.png"
        p.write_bytes(b"x")
        paths.append(p)
        time.sleep(0.01)
...
def test_list_recent_screenshots_ignores_unrelated_files(tmp_path: Path):
    keep = tmp_path / "kuick-pic-2026-01-01-12-00-00.jpg"
...
def test_list_recent_screenshots_includes_legacy_prefixes(tmp_path: Path):
    current = tmp_path / "kuick-pic-2026-01-02-00-00-00.png"
    kquick = tmp_path / "kquick-pic-2026-01-01-00-00-00.png"
    current.write_bytes(b"\x89PNG\r\n\x1a\n")
    time.sleep(0.02)
    kquick.write_bytes(b"\x89PNG\r\n\x1a\n")

    result = list_recent_screenshots(tmp_path, limit=5)

    assert set(result) == {current, kquick}
